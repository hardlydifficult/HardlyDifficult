<template>
  <div>
  {{test.fetchedCoinBalance}}
  {{hello}}
  <div v-for="t in test.transactions.edges" v-bind:key="t.node.hash">
    {{t.node.hash}}
  </div>
...
export default {
  ...
  computed: {
    web3() {
      return new Web3("https://blockscout.com/poa/xdai/api/eth-rpc")
    }
  },
  asyncComputed: {
    async hello() {
      return await abiDecoder.decodeMethod(this.test.transactions.edges[0].node.input)
    }
    }
}
</script>